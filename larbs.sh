#!/bin/sh
# Ayham's Auto Rice Boostrapping Script (AARBS)
# moded for Ayham Alhalaibi <https://ayhamcloud.de>
...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/ayham291/dotfiles.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/ayham291/AARBS/master/progs.csv"
...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "Welcome to Ayham's Auto-Rice Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-Ayham" 10 60
...
usercheck() { \
	...
	dialog --colors --title "WARNING!" --yes-label "CONTINUE" --no-label "No wait..." --yesno "The user \`$name\` already exists on this system. AARBS can install for a user already existing, but it will \\Zboverwrite\\Zn any conflicting settings/dotfiles on the user account.\\n\\nAARBS will \\Zbnot\\Zn overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that AARBS will change $name's password to the one you just gave." 14 70
	}
...
maininstall() { # Installs all needed programs from main repo.
	dialog --title "AARBS Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 5 70
	installpkg "$1"
	}
...
gitmakeinstall() {
	...
	dialog --title "AARBS Installation" --infobox "Installing \`$progname\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
aurinstall() { \
	dialog --title "AARBS Installation" --infobox "Installing \`$1\` ($n of $total) from the AUR. $1 $2" 5 70
	echo "$aurinstalled" | grep -q "^$1$" && return 1
	sudo -u "$name" $aurhelper -S --noconfirm "$1" >/dev/null 2>&1
	}

pipinstall() { \
	dialog --title "AARBS Installation" --infobox "Installing the Python package \`$1\` ($n of $total). $1 $2" 5 70
...
dialog --title "AARBS Installation" --infobox "Synchronizing system time to ensure successful and secure installation of software..." 4 70
...
dialog --title "AARBS Installation" --infobox "Finally, installing \`libxft-bgra\` to enable color emoji in suckless software without crashes." 5 70
